...
class ReadingTrueFalse extends BaseQuestionEditor
{
    ...
    protected function initAnswers()
    {
        foreach ($this->group->questions as $q) {
            ...
            if ($correctOpt && ($correctOpt->content === '×' || $correctOpt->content === '✕')) {
                $this->trueFalseAnswers[$q->id] = 0;
            } else {
                $this->trueFalseAnswers[$q->id] = 1;
            }
        }
    }
        ...
        DB::transaction(function () {
            foreach ($this->group->questions as $q) {
                ...
                foreach ($q->options as $opt) {
                    ...
                    if ($opt->content === '✕' || $opt->content === '×') $opt->is_correct = ($val == 0);
                    $opt->save();
                }
            }
            parent::saveGroup();
    ...
    {
        return view('livewire.exam-builder.types.reading-true-false');
    }
}
