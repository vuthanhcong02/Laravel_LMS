<!DOCTYPE html>
<html lang="vi" class="h-full" :class="{ 'dark': darkMode }" x-data="authRegisterPage">
<head>
    <meta charset="UTF-8">
                ...
                <div class="lms-card p-6 sm:p-10 space-y-5">
                    ...
                    <form method="POST" action="{{ route('register') }}" class="space-y-3.5">
                        ...
                        <button type="submit" class="w-full py-3 bg-[#e07a5f] hover:bg-[#c86349] text-white font-bold rounded-xl text-xs sm:text-sm shadow-md transition-all btn-tactile hover:shadow-lg mt-1">
                            {{ __('Tạo tài khoản miễn phí') }}
                        </button>
                    </form>
                    <div class="relative flex items-center justify-center my-3">
                        <div class="border-t border-[#e8e2d9] dark:border-[#2d2926] w-full"></div>
                        <span class="bg-white dark:bg-[#181615] px-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider absolute">{{ __('Hoặc') }}</span>
                    </div>
                    <!-- Social Register (Google) -->
                    <div>
                        <a href="{{ route('socialite.redirect', 'google') }}" class="w-full flex items-center justify-center gap-2.5 py-3 px-4 rounded-xl border border-[#e8e2d9] dark:border-[#2d2926] bg-white dark:bg-[#201d1b] text-xs sm:text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-[#282422] transition-colors btn-tactile shadow-xs cursor-pointer">
