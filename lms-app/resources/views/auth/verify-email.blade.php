...
<body class="h-full bg-[#f8f6f3] dark:bg-[#0e0c0b] text-slate-800 dark:text-slate-100 flex flex-col transition-colors duration-200">
            ...
            <div class="relative">
                <button @click="langOpen = !langOpen" @click.outside="langOpen = false" class="flex items-center gap-2 px-3 py-2 rounded-xl bg-white dark:bg-slate-800 border border-[#e8e2d9] dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-200 hover:border-slate-300 transition-all btn-tactile">
                    ...
                    <template x-if="currentLang === 'English'">
                        <svg class="w-5 h-3.5 rounded-xs object-cover border border-slate-200 shrink-0" viewBox="0 0 60 30">
                            <clipPath id="s_uk_rst"><path d="M0,0 v30 h60 v-30 z"/></clipPath>
                            <clipPath id="t_uk_rst"><path d="M30,15 L60,0 v30 z M30,15 L0,30 v-30 z M30,15 L0,0 h60 z M30,15 L60,30 h-60 z"/></clipPath>
                            <g clip-path="url(#s_uk_rst)">
                                <path d="M0,0 L60,30 M60,0 L0,30" stroke="#fff" stroke-width="6"/>
                                <path d="M0,0 L60,30 M60,0 L0,30" stroke="#cf142b" stroke-width="4" clip-path="url(#t_uk_rst)"/>
                ...
                <div x-show="langOpen" 
                    ...
                    <button @click="currentLang = 'English'; langOpen = false" class="w-full flex items-center gap-2.5 px-3.5 py-2.5 hover:bg-[#fff2ee] dark:hover:bg-slate-800 font-bold transition-colors" :class="currentLang === 'English' ? 'text-[#e07a5f]' : 'text-slate-700 dark:text-slate-300'">
                        <svg class="w-5 h-3.5 rounded-xs object-cover border border-slate-200 shrink-0" viewBox="0 0 60 30">
                            <clipPath id="s_uk2_rst"><path d="M0,0 v30 h60 v-30 z"/></clipPath>
                            <clipPath id="t_uk2_rst"><path d="M30,15 L60,0 v30 z M30,15 L0,30 v-30 z M30,15 L0,0 h60 z M30,15 L60,30 h-60 z"/></clipPath>
                            <g clip-path="url(#s_uk2_rst)">
                                <path d="M0,0 L60,30 M60,0 L0,30" stroke="#fff" stroke-width="6"/>
                                <path d="M0,0 L60,30 M60,0 L0,30" stroke="#cf142b" stroke-width="4" clip-path="url(#t_uk2_rst)"/>
                                <path d="M30,0 v30 M0,15 h60" stroke="#fff" stroke-width="10"/>
                                <path d="M30,0 v30 M0,15 h60" stroke="#cf142b" stroke-width="6"/>
                            </g>
                        </svg>
    ...
    <!-- Nội dung chính Form Đặt Lại Mật Khẩu -->
            ...
            <div class="lms-card p-6 sm:p-10 space-y-6">
                ...
                <div class="text-center space-y-2">
                    <div class="w-12 h-12 rounded-2xl bg-[#fff2ee] dark:bg-[#2c221e] text-[#e07a5f] border border-[#fcdccf] dark:border-[#42271f] flex items-center justify-center text-xl mx-auto shadow-xs">
                        <i class="fa-solid fa-envelope-circle-check text-[#e07a5f]"></i>
                    </div>
                    <h2 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Xác thực Email ✨</h2>
                    <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-normal leading-relaxed mt-2">
                        Cảm ơn bạn đã đăng ký! Trước khi bắt đầu, vui lòng xác thực địa chỉ email bằng cách nhấp vào liên kết chúng tôi vừa gửi cho bạn. Nếu bạn không nhận được email, chúng tôi có thể gửi lại một liên kết khác.
                    </p>
                </div>

                @if (session('status') == 'verification-link-sent')
                    <div class="p-3.5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/50 text-emerald-600 dark:text-emerald-400 text-xs font-semibold flex items-center gap-2">
                        <i class="fa-solid fa-circle-check text-sm shrink-0"></i>
                        <span>Một liên kết xác thực mới đã được gửi đến email bạn dùng khi đăng ký.</span>
                    </div>
                @endif
                ...
                <div class="flex flex-col gap-3 pt-2">
                    ...
                    <form method="POST" action="{{ route('verification.send') }}">
                        @csrf
                        <button type="submit" class="w-full py-3 bg-[#e07a5f] hover:bg-[#c86349] text-white font-bold rounded-xl text-xs sm:text-sm shadow-md transition-all btn-tactile hover:shadow-lg flex items-center justify-center gap-2 cursor-pointer">
                            <i class="fa-regular fa-paper-plane text-xs"></i> Gửi lại email xác thực
                        </button>
                    </form>
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="text-center">
                        @csrf
                        <button type="submit" class="font-bold text-xs text-[#e07a5f] hover:underline cursor-pointer inline-flex items-center gap-2 mt-2">
