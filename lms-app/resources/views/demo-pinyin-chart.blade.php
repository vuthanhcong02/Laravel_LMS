<!DOCTYPE html>
<html lang="vi" class="h-full" :class="{ 'dark': darkMode }" x-data="{ sidebarOpen: false, sidebarCollapsed: false, isLoggedIn: true, langOpen: false, currentLang: 'Việt Nam', darkMode: false, selectedInitial: 'b', selectedFinal: 'a', selectedTone: 1, socialDockExpanded: true, initials: ['b', 'p', 'm', 'f', 'd', 't', 'n', 'l', 'g', 'k', 'h', 'j', 'q', 'x', 'zh', 'ch', 'sh', 'r', 'z', 'c', 's'], finals: ['a', 'o', 'e', 'i', 'u', 'ü', 'ai', 'ei', 'ao', 'ou', 'an', 'en', 'ang', 'eng', 'ong'] }">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Bảng Phiên âm Pinyin Interactive - Tiếng Trung XIAOMU LMS</title>
    <meta name="description" content="Tra cứu bảng phiên âm Pinyin tiếng Trung đầy đủ Thanh mẫu, Vận mẫu và 4 Thanh điệu.">
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-5 no-scrollbar">
                <div>
                    ...
                    <div class="space-y-1">
                        ...
                        <a href="{{ url('/demo-pinyin-chart') }}" class="flex items-center gap-3 rounded-xl text-sm nav-item-active transition-all btn-tactile" :class="sidebarCollapsed ? 'justify-center p-2.5' : 'px-3.5 py-2.5'" :title="sidebarCollapsed ? 'Bảng Pinyin' : ''">
                            <i class="fa-solid fa-table-cells text-base w-5 text-center shrink-0"></i>
                    ...
                    <div class="relative">
                        <button @click="langOpen = !langOpen" @click.outside="langOpen = false" class="flex items-center gap-2 px-3 py-2 rounded-xl bg-white dark:bg-slate-800 border border-[#e8e2d9] dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-200 hover:border-slate-300 transition-all btn-tactile">
                            ...
                            <template x-if="currentLang === 'English'">
                                <svg class="w-5 h-3.5 rounded-xs object-cover border border-slate-200 shrink-0" viewBox="0 0 60 30"><clipPath id="s_uk_p7"><path d="M0,0 v30 h60 v-30 z"/></clipPath><clipPath id="t_uk_p7"><path d="M30,15 L60,0 v30 z M30,15 L0,30 v-30 z M30,15 L0,0 h60 z M30,15 L60,30 h-60 z"/></clipPath><g clip-path="url(#s_uk_p7)"><path d="M0,0 L60,30 M60,0 L0,30" stroke="#fff" stroke-width="6"/><path d="M0,0 L60,30 M60,0 L0,30" stroke="#cf142b" stroke-width="4" clip-path="url(#t_uk_p7)"/><path d="M30,0 v30 M0,15 h60" stroke="#fff" stroke-width="10"/><path d="M30,0 v30 M0,15 h60" stroke="#cf142b" stroke-width="6"/></g></svg>
                            </template>
