...
@section('header-left')
    <div class="relative w-full">
        ...
        <input type="text" placeholder="Tra cứu âm Pinyin..."
            class="w-full bg-[#f8f6f3] dark:bg-slate-800 border border-[#e8e2d9] dark:border-slate-700 rounded-xl pl-10 pr-10 sm:pr-12 py-2 text-xs sm:text-sm text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:border-[#e07a5f] focus:ring-2 focus:ring-[#e07a5f]/20 transition-all">
        <span
...
@section('content')
    ...
    <div
        ...
        <div class="flex items-center gap-6">
            <div
                class="w-16 h-16 rounded-2xl bg-[#e07a5f] text-white font-bold text-3xl flex items-center justify-center shadow-lg shadow-[#e07a5f]/25 shrink-0">
            ...
            <div class="space-y-1">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-bold text-slate-900 dark:text-white"
                        x-text="selectedInitial + ' + ' + selectedFinal">b + a</span>
                </div>
                <h2 class="text-lg font-bold text-slate-900 dark:text-white">Thanh 1 (Thanh bằng): <span
                        class="text-[#e07a5f]" x-text="selectedInitial + 'ā'">bā</span></h2>
    ...
    <div class="lms-card p-5 space-y-3">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <button @click="selectedTone = 1"
                :class="selectedTone === 1 ? 'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2a221f] text-[#e07a5f] font-bold' :
                    'border-[#e8e2d9] dark:border-[#2d2926] text-slate-700 dark:text-slate-300'"
                class="p-3 rounded-xl border text-left btn-tactile">
                <div class="font-bold">Thanh 1 (¯)</div>
                <div class="text-[10px] text-slate-400">Cao bằng (5-5)</div>
            </button>
            <button @click="selectedTone = 2"
                :class="selectedTone === 2 ? 'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2a221f] text-[#e07a5f] font-bold' :
                    'border-[#e8e2d9] dark:border-[#2d2926] text-slate-700 dark:text-slate-300'"
                class="p-3 rounded-xl border text-left btn-tactile">
                <div class="font-bold">Thanh 2 (ˊ)</div>
                <div class="text-[10px] text-slate-400">Sắc lên (3-5)</div>
            </button>
            <button @click="selectedTone = 3"
                :class="selectedTone === 3 ? 'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2a221f] text-[#e07a5f] font-bold' :
                    'border-[#e8e2d9] dark:border-[#2d2926] text-slate-700 dark:text-slate-300'"
                class="p-3 rounded-xl border text-left btn-tactile">
                <div class="font-bold">Thanh 3 (ˇ)</div>
                <div class="text-[10px] text-slate-400">Hỏi xuống lên (2-1-4)</div>
            </button>
            <button @click="selectedTone = 4"
                :class="selectedTone === 4 ? 'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2a221f] text-[#e07a5f] font-bold' :
                    'border-[#e8e2d9] dark:border-[#2d2926] text-slate-700 dark:text-slate-300'"
                class="p-3 rounded-xl border text-left btn-tactile">
    ...
    <div class="lms-card p-5 space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
            <template x-for="init in initials" :key="init">
                <button @click="selectedInitial = init"
                    :class="selectedInitial === init ? 'bg-[#e07a5f] text-white font-bold' :
                        'bg-[#f8f6f3] dark:bg-[#201d1b] text-slate-700 dark:text-slate-300 border border-[#e8e2d9] dark:border-[#2d2926]'"
                    class="w-10 h-10 rounded-xl text-sm font-bold flex items-center justify-center btn-tactile">
    ...
    <div class="lms-card p-5 space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
            <template x-for="fin in finals" :key="fin">
                <button @click="selectedFinal = fin"
                    :class="selectedFinal === fin ? 'bg-[#e07a5f] text-white font-bold' :
                        'bg-[#f8f6f3] dark:bg-[#201d1b] text-slate-700 dark:text-slate-300 border border-[#e8e2d9] dark:border-[#2d2926]'"
                    class="px-3.5 h-10 rounded-xl text-sm font-bold flex items-center justify-center btn-tactile">
