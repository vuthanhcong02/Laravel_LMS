<div class="space-y-6">
    <div class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
        ...
        <textarea wire:model.defer="group.passage_text" rows="3"
            placeholder="Ví dụ: 第一部分，一共5个题。请将词语组合成完整的句子。"
    ...
    <div>
        <h4 class="text-sm font-black text-slate-600 dark:text-slate-300 uppercase tracking-wide mb-3">
            Danh sách Câu hỏi Sắp xếp từ ({{ $group->questions->count() }})
        </h4>
                ...
                <div class="pl-11 space-y-3">
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase mb-1">
                            Các từ cần sắp xếp (phân tách bởi dấu gạch chéo <code class="text-primary font-bold">/</code>)
                        </label>
                        <input type="text"
                            wire:model.defer="questionTitles.{{ $index }}"
                            placeholder="Ví dụ: 最 / 吃 / 我 / 西瓜 / 爱"
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-emerald-600 dark:text-emerald-400 uppercase mb-1">
                            Câu đáp án đúng hoàn chỉnh
                        </label>
                        <input type="text"
                            wire:model.defer="correctAnswers.{{ $question->id }}"
                            placeholder="Ví dụ: 我最爱吃西瓜。"
                            class="w-full text-sm rounded-lg border border-emerald-300 dark:border-emerald-700 bg-emerald-50/40 dark:bg-emerald-950/20 text-emerald-900 dark:text-emerald-200 focus:ring-emerald-500 focus:border-emerald-500 px-3 py-2 font-bold zh-text text-base">
                    </div>
