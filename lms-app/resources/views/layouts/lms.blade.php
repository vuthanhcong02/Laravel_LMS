...
<html lang="vi" class="h-full" :class="{ 'dark': darkMode }"
    @open-auth-modal.window="openAuthModal($event.detail?.tab, $event.detail?.redirect)"
    @open-contact-modal.window="contactModalOpen = true"
    x-data="lmsApp({ isLoggedIn: {{ auth()->check() ? 'true' : 'false' }}, authModalOpen: {{ auth()->guest() && ($errors->any() || session('status')) ? 'true' : 'false' }}, authModalTab: '{{ session('auth_tab') ?? (old('first_name') ? 'register' : 'login') }}' })">
<head>
    <script>
...
<body
    ...
    <div class="flex h-full w-full overflow-hidden relative">
        ...
        <main class="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
            ...
            @hasSection('sub-header')
                <div class="px-4 sm:px-6 @if(!View::hasSection('hide_sidebar')) lg:pr-16 @endif pb-0 pt-4 shrink-0 bg-[#f8f6f3] dark:bg-[#0e0c0b] z-20 transition-all duration-300">
            ...
            @endif
            <!-- Page Body -->
            <div class="flex-1 overflow-y-auto no-scrollbar transition-all duration-300 space-y-6 px-4 sm:px-6 pt-8 pb-6 @if(!View::hasSection('hide_sidebar')) lg:pr-16 @endif">
    ...
    <div class="fixed right-0 top-1/2 -translate-y-1/2 z-50 hidden lg:flex flex-col items-end">
        <div
    ...
    @if (session('status') && auth()->check())
        <div x-data="lmsToast(5000)" x-show="showToast"
            ...
            <div>
                <p class="text-xs font-bold text-slate-800 dark:text-white">{{ __('Thành công') }}</p>
