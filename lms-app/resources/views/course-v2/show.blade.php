...
@section('header-left')
    @if(isset($currentLevel) && isset($currentLesson))
        ...
        <div class="flex items-center text-xs text-slate-500 font-semibold truncate min-w-0">
            <a href="{{ route('home') }}" class="hidden sm:inline-flex items-center hover:text-[#e07a5f] transition-colors shrink-0">
            ...
            <a href="{{ route('courses') }}" class="hidden md:inline-flex items-center hover:text-[#e07a5f] transition-colors shrink-0">
                {{ __('Khóa học') }}
            </a>
            <i class="hidden md:inline-block fa-solid fa-chevron-right text-[9px] mx-2 text-slate-400 shrink-0"></i>
            <a href="{{ route('courses.level', ['levelSlug' => $currentLevel->slug]) }}" class="hover:text-[#e07a5f] transition-colors shrink-0 font-medium">
                {{ $currentLevel->title }}
            </a>
...
@endsection
@section('sub-header')
    <div x-data="lessonStudyApp({ activeTab: '{{ $activeTab }}' })" class="lms-card p-1.5 sm:p-2 bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926] rounded-2xl flex flex-col lg:flex-row lg:items-center justify-between gap-2 shadow-xs">
        ...
        <div x-show="activeTab === 'tu-vung'" style="display: none;" class="flex items-center gap-1 p-1 bg-[#fcfaf7] dark:bg-[#23201e] border border-[#e8e2d9] dark:border-[#2d2926] rounded-xl shrink-0 overflow-x-auto no-scrollbar max-w-full">
            <button @click="$store.lesson.vocabSubView = 'table';" 
                    :class="($store.lesson ? $store.lesson.vocabSubView : 'table') === 'table' ? 'bg-white dark:bg-[#181615] text-[#e07a5f] font-bold shadow-xs' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 font-semibold'" 
            ...
            <button @click="$store.lesson.vocabSubView = 'flashcard'; if($store.lesson) { $store.lesson.fcIndex = 0; $store.lesson.fcFlipped = false; }" 
                    :class="($store.lesson ? $store.lesson.vocabSubView : 'table') === 'flashcard' ? 'bg-white dark:bg-[#181615] text-[#e07a5f] font-bold shadow-xs' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 font-semibold'" 
                    class="px-2.5 py-1.5 sm:px-3 rounded-lg text-xs transition-all btn-tactile flex items-center gap-1.5 whitespace-nowrap shrink-0">
                <i class="fa-solid fa-layer-group"></i>
                <span>{{ __('Flashcard') }}</span>
            </button>
            <button @click="$store.lesson.vocabSubView = 'match';" 
                    :class="($store.lesson ? $store.lesson.vocabSubView : 'table') === 'match' ? 'bg-white dark:bg-[#181615] text-[#e07a5f] font-bold shadow-xs' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 font-semibold'" 
                    class="px-2.5 py-1.5 sm:px-3 rounded-lg text-xs transition-all btn-tactile flex items-center gap-1.5 whitespace-nowrap shrink-0">
                <i class="fa-solid fa-puzzle-piece"></i>
                <span>{{ __('Nối từ') }}</span>
            </button>
            <button @click="$store.lesson.vocabSubView = 'quiz';" 
                    :class="($store.lesson ? $store.lesson.vocabSubView : 'table') === 'quiz' ? 'bg-white dark:bg-[#181615] text-[#e07a5f] font-bold shadow-xs' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 font-semibold'" 
                    class="px-2.5 py-1.5 sm:px-3 rounded-lg text-xs transition-all btn-tactile flex items-center gap-1.5 whitespace-nowrap shrink-0">
                <i class="fa-solid fa-clipboard-question"></i>
                <span>{{ __('Trắc nghiệm') }}</span>
            </button>
            <button @click="$store.lesson.vocabSubView = 'typing';" 
                    :class="($store.lesson ? $store.lesson.vocabSubView : 'table') === 'typing' ? 'bg-white dark:bg-[#181615] text-[#e07a5f] font-bold shadow-xs' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 font-semibold'" 
                    class="px-2.5 py-1.5 sm:px-3 rounded-lg text-xs transition-all btn-tactile flex items-center gap-1.5 whitespace-nowrap shrink-0">
