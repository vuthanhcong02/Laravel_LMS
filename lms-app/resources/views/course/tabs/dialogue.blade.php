...
<div x-show="activeTab === 'hoi-thoai'" x-transition:enter="transition ease-out duration-200" style="display: none;">
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 text-left">
                ...
                @foreach ($currentLesson->dialogueSections as $sIdx => $section)
                    <div x-data="{ dialogues: @js($section->dialogues) }">
                        <div x-show="currentDialogueSectionIdx === {{ $sIdx }}"
                            class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-150/80 dark:border-slate-800/80 p-5 shadow-sm space-y-4 flex flex-col">
                            <!-- Top Dialogue section header with audio player -->
                            <div
                                class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800/80">
                                ...
                                <div class="w-full md:w-1/2 lg:w-[350px]">
                                    <audio
                                        src="{{ $section->audio_path ? (str_starts_with($section->audio_path, 'http') ? $section->audio_path : (str_starts_with($section->audio_path, 'audio/') ? '/storage/hsk_media/' . $section->audio_path : '/storage/hsk_media/audio/' . $section->audio_path)) : '' }}"
                                        controls class="w-full h-10 outline-none"></audio>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-2 mb-4">
                                <button
                                    ...
                                    :class="modePinyin ? 'bg-primary text-white border-primary shadow-sm shadow-primary/20' :
                                        'bg-white text-primary border-primary/30 hover:border-primary hover:bg-primary/5 dark:bg-slate-800 dark:border-primary/40 dark:text-primary'">
                                ...
                                <button
                                    ...
                                    class="px-4 py-1.5 rounded-2xl text-[14px] font-bold transition-all duration-200 flex items-center gap-1.5 outline-none border"
                                    :class="modeHanyu ? 'bg-primary text-white border-primary shadow-sm shadow-primary/20' :
                                        'bg-white text-primary border-primary/30 hover:border-primary hover:bg-primary/5 dark:bg-slate-800 dark:border-primary/40 dark:text-primary'">
                                ...
                                <button
                                    ...
                                    :class="modeGo ? 'bg-primary text-white border-primary shadow-sm shadow-primary/20' :
                                        'bg-white text-primary border-primary/30 hover:border-primary hover:bg-primary/5 dark:bg-slate-800 dark:border-primary/40 dark:text-primary'">
                                ...
                                <button
                                    ...
                                    class="px-4 py-1.5 rounded-2xl text-[14px] font-bold transition-all duration-200 flex items-center gap-1.5 outline-none border"
                                    :class="modeNghe ? 'bg-primary text-white border-primary shadow-sm shadow-primary/20' :
                                        'bg-white text-primary border-primary/30 hover:border-primary hover:bg-primary/5 dark:bg-slate-800 dark:border-primary/40 dark:text-primary'">
                                ...
                                <button
                                    ...
                                    class="px-4 py-1.5 rounded-2xl text-[14px] font-bold transition-all duration-200 flex items-center gap-1.5 outline-none border"
                                    :class="modeDich ? 'bg-primary text-white border-primary shadow-sm shadow-primary/20' :
                                        'bg-white text-primary border-primary/30 hover:border-primary hover:bg-primary/5 dark:bg-slate-800 dark:border-primary/40 dark:text-primary'">
                            ...
                            <div x-show="modeNghe || modeGo || modeDich" class="mt-6 mb-4 w-full">
                                <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 relative">
                                    ...
                                    <div x-show="modeNghe" class="text-center space-y-4 mb-6">
                                        ...
                                        <button x-show="dialogues[quizIndex].audio_path"
                                            ...
                                            <div class="flex flex-wrap items-end gap-x-1.5 gap-y-2 leading-none">
                                                <template
                                                    ...
                                                    <div class="flex flex-col items-center">
                                                        <span
                                                            class="text-[13px] text-slate-500 dark:text-slate-400 font-pinyin font-normal leading-none mb-1.5"
                                                            x-html="pair.p === '.' ? '&nbsp;' : pair.p"></span>
                                                        <span
                                                            class="font-chinese text-[24px] font-black text-slate-800 dark:text-slate-200 leading-none"
                                                            x-text="pair.h"></span>
                                                    </div>
                                                </template>
                                            </div>
                                    ...
                                    <div class="mb-6 relative">
                                        ...
                                        <!-- Result Icons -->
                                        <div x-show="quizStatus === 'correct'"
                                            class="absolute right-4 top-1/2 -translate-y-1/2 text-emerald-500">
                                            <span class="material-symbols-outlined text-2xl">check_circle</span>
                                        </div>
                                        <div x-show="quizStatus === 'incorrect'"
                                            class="absolute right-4 top-1/2 -translate-y-1/2 text-red-500">
                                    ...
                                    <div class="flex flex-wrap items-center justify-between gap-4">
                                        <div class="flex gap-2">
                                            ...
                                            <button @click="quizRetry()"
                                                class="px-5 py-2.5 rounded-lg border border-slate-300 font-bold text-sm transition-all focus:outline-none text-slate-700 hover:bg-slate-50 active:scale-95">
                                                Làm lại
                                            </button>
                                        </div>

                                        <button @click="modeNghe = false; modeGo = false; modeDich = false;"
                                            class="px-5 py-2.5 rounded-lg border border-slate-300 font-bold text-sm transition-all focus:outline-none text-slate-700 hover:bg-slate-50 active:scale-95">
                                            Thoát
                                        </button>
                                    </div>
                                    ...
                                    <!-- Footer counter -->
                                    <div
                                        class="mt-6 pt-4 border-t border-slate-100 text-left text-sm text-slate-400 font-medium">
                                        Câu <span x-text="quizIndex + 1"></span>/<span
                                            x-text="dialogues.length"></span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4 pt-3 flex flex-col" x-show="!modeNghe && !modeGo && !modeDich">
                                ...
                                @foreach ($section->dialogues as $lineIdx => $line)
                                    ...
                                    @endphp
                                    <div class="flex gap-2 max-w-md items-end {{ $style['align'] }}">
                                        <!-- Avatar Icon -->
                                        <div
                                        ...
                                        <div class="space-y-1 relative">
                                            <div
                                                ...
                                                <div class="flex justify-between items-center mb-1 gap-4">
                                                    <span
                                                        class="text-[9px] font-black uppercase text-slate-400">{{ $line->role }}</span>
                                                    @if ($line->audio_path)
                                                        <button @click="playAudio('{{ $line->audio_path }}')"
                                                            ...
                                                            <span
                                                                class="material-symbols-outlined text-[11px]">volume_up</span>
                                                        </button>
                                                    @endif
                                                </div>
                                                ...
                                                <div x-data="{ l_c: {{ json_encode($line->character) }}, l_p: {{ json_encode($line->pinyin) }} }">
                                                    <template
                                                        x-if="window.alignPinyin(l_c, l_p, currentLevelObj?.level_code)">
                                                        <div class="flex flex-wrap items-end gap-x-1.5 gap-y-2 leading-none"
                                                            x-show="modePinyin && modeHanyu">
                                                            <template
                                                                x-for="(pair, idx) in window.alignPinyin(l_c, l_p, currentLevelObj?.level_code)"
                                                                :key="idx">
                                                                <div class="flex flex-col items-center">
                                                                    <span
                                                                        class="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 font-pinyin font-normal leading-none mb-1"
                                                                        x-html="pair.p === '.' ? '&nbsp;' : pair.p"></span>
                                                                    <span
                                                                        class="font-chinese text-base sm:text-lg font-bold text-slate-800 dark:text-slate-200 leading-none"
                                                                        x-text="pair.h"></span>
                                                                </div>
                                                            </template>
                                                        </div>
                                                    ...
                                                    <template
                                                        ...
                                                        <div>
                                                            <p x-show="modePinyin && modeHanyu"
                                                                class="text-[11px] sm:text-xs text-slate-400 dark:text-slate-550 italic mb-0.5 block"
                                                                x-text="l_p"></p>
                                                            <p x-show="modeHanyu && modePinyin"
                                                                class="text-sm sm:text-base font-bold tracking-wide leading-relaxed block"
                                                                x-text="l_c"></p>
                                                        </div>
                                                    </template>

                                                    <p x-show="modePinyin && !modeHanyu"
                                                        class="text-[11px] sm:text-xs text-slate-400 dark:text-slate-550 italic block"
                                                        x-text="l_p"></p>
                                                    <p x-show="modeHanyu && !modePinyin"
                                                        class="text-sm sm:text-base font-bold tracking-wide leading-relaxed block"
                                                        x-text="l_c"></p>
        ...
        <div class="lg:col-span-1 order-1 lg:order-2 space-y-4">
            <div
                class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm rounded-2xl p-4 text-left">
                <h5
                    class="text-sm font-black text-slate-800 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-3 mb-4 flex items-center gap-2">
                    <span class="material-symbols-outlined text-[18px] text-primary">route</span>
                    <span>Điều hướng bài khóa</span>
                </h5>

                <div
                    class="flex flex-col space-y-1 relative before:content-[''] before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-px before:bg-slate-100 dark:before:bg-slate-800">
                    @if (isset($currentLesson) && $currentLesson->dialogueSections)
                        @foreach ($currentLesson->dialogueSections as $sIdx => $section)
                            @php
                            ...
                            <button @click="currentDialogueSectionIdx = {{ $sIdx }}"
                                class="group relative flex items-center gap-3 py-2 text-left z-10 cursor-pointer"
                                title="{{ $section->title }}">
                                <div class="h-10 w-10 shrink-0 rounded-full flex items-center justify-center text-xs font-bold transition-all border-4 border-white dark:border-slate-900"
                                ...
                                <div class="flex flex-col min-w-0 flex-1">
                                    <span class="text-sm font-bold transition-colors line-clamp-2"
                                        :class="currentDialogueSectionIdx === {{ $sIdx }} ? 'text-primary' :
                                            'text-slate-500 dark:text-slate-400 group-hover:text-primary dark:group-hover:text-primary-light'"
