        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('flashcardComponent', () => ({
                ...
                getProgressPercentage() {
                    ...
                    let rememberedInCurrent = this.rememberedWords.filter(w => wordStrings.includes(w)).length;
    ...
    <section id="flashcard-section" class="pt-8 lg:pt-12 pb-24 bg-transparent" x-data="flashcardComponent()">
        <div class="mx-auto max-w-7xl px-6">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
                ...
                <div class="hidden lg:flex lg:col-span-1 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200/50 dark:border-slate-700/60 shadow-xl shadow-slate-100/30 dark:shadow-none p-5 relative z-10 flex-col gap-6">
                ...
                <div class="col-span-1 lg:col-span-3 flex flex-col items-center justify-center gap-4 relative z-10 w-full">
                    ...
                    <div class="w-full max-w-3xl flex lg:hidden justify-between items-center bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 px-5 py-3 rounded-2xl shadow-sm mb-1">
                        <span class="flex items-center gap-1.5">
                            <span class="h-2 w-2 rounded-full bg-primary animate-ping"></span>
                            <span class="font-extrabold text-xs text-primary" x-text="studyMode === 'level' ? 'HSK ' + activeLevel : activeTopic"></span>
                        </span>
                        <button 
                    ...
                    <template x-if="currentWords().length > 0">
                        <div class="w-full flex flex-col items-center justify-center gap-4">
                            ...
                            <div class="w-full max-w-3xl flex flex-col gap-1.5 mb-1 px-1">
                                <div class="flex justify-between items-center text-[9px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">
                                    <span>Tiến độ ghi nhớ</span>
                                    <span class="text-primary font-black" x-text="getProgressPercentage() + '%'"></span>
                                </div>
                                <div class="w-full h-1.5 bg-slate-100 dark:bg-slate-900 border border-slate-200/10 dark:border-slate-800/80 rounded-full overflow-hidden shadow-inner">
                                    <div 
                                        class="h-full bg-gradient-to-r from-primary via-primary/95 to-primary/80 rounded-full transition-all duration-500 ease-out"
                            ...
                            <div class="w-full max-w-3xl h-[380px] md:h-[420px] perspective">
                                ...
                                >
                                    ...
                                    <div class="backface-hidden w-full h-full bg-white/95 dark:bg-slate-800/95 backdrop-blur-md rounded-[32px] border border-primary/10 dark:border-slate-700/60 shadow-xl hover:shadow-2xl hover:shadow-primary/5 hover:border-primary/50 dark:hover:border-primary/40 transition-all duration-300 flex flex-col justify-between p-6 absolute top-0 left-0">
                                        <!-- Card Header Info -->
                                        <div class="flex justify-between items-center w-full">
                                            <!-- Mark Learned Button (Premium Minimalist Check) -->
                                            <div class="relative group">
                                                <button 
                                                    @click.stop="markAsRemembered(currentWord().word, currentWord().id)" 
                                                ...
                                                <!-- Custom Tooltip -->
                                                <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 scale-0 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-200 origin-bottom pointer-events-none z-50">
                                                    <div class="bg-slate-900 dark:bg-slate-750 text-white text-[10px] font-bold py-1.5 px-3 rounded-lg shadow-md text-center leading-normal">
                                                        Đánh dấu đã thuộc (từ này sẽ ẩn và không xuất hiện lại nữa)
                                                    </div>
                                                    <div class="w-2 h-2 bg-slate-900 dark:bg-slate-750 rotate-45 absolute top-full left-1/2 -translate-x-1/2 -translate-y-1/2"></div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="backface-hidden rotate-y-180 w-full h-full bg-white/95 dark:bg-slate-800/95 backdrop-blur-md rounded-[32px] border border-primary/10 dark:border-slate-700/60 shadow-xl hover:shadow-2xl hover:shadow-primary/5 hover:border-primary/50 dark:hover:border-primary/40 transition-all duration-300 flex flex-col justify-between p-6 absolute top-0 left-0">
                                        ...
                                        <div class="flex flex-col gap-4 my-auto text-left w-full overflow-y-auto max-h-[240px] md:max-h-[260px] pr-1.5 no-scrollbar">
                                            <div class="flex items-baseline gap-3">
                                                <h4 class="text-2xl md:text-3xl font-extrabold text-primary" x-text="currentWord().word"></h4>
                                                <p class="text-base md:text-lg font-bold text-slate-400 dark:text-slate-500 tracking-wide" x-text="'[' + currentWord().pinyin + ']'"></p>
                                            </div>
                                            
                                            <!-- Meaning -->
                                            <div class="bg-primary/10 dark:bg-primary/20 px-4 py-2.5 rounded-2xl border-l-4 border-primary shadow-sm">
                                                <p class="text-sm md:text-base font-bold text-slate-800 dark:text-white" x-text="currentWord().meaning"></p>
                                            </div>

                                            <!-- Example Section -->
                                            <template x-if="currentWord().example">
                                                <div class="bg-slate-50/50 dark:bg-slate-900/40 p-4 rounded-2xl border border-slate-100/85 dark:border-slate-800/80 flex flex-col gap-3">
                                                    <div class="flex items-center justify-between mb-0.5">
                                                        <p class="text-[9px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Ví dụ minh họa:</p>
                                                        <button 
                                                            @click.stop="
                                                                if ('speechSynthesis' in window) {
                                                                    window.speechSynthesis.cancel();
                                                    ...
                                                    <template x-for="(ex, index) in currentWord().example.split('\n')" :key="index">
                                                        <div class="flex flex-col gap-1 border-b last:border-b-0 border-slate-100/80 dark:border-slate-800/60 pb-2 last:pb-0">
                                                            <p class="text-xs md:text-sm font-bold text-slate-800 dark:text-white tracking-wide" x-text="(index + 1) + '. ' + ex"></p>
                                                            <p class="text-[11px] md:text-xs text-slate-500 dark:text-slate-400" x-text="currentWord().example_meaning.split('\n')[index] || ''"></p>
                                                        </div>
                                                    </template>
                                                </div>
                                            </template>
                                        ...
                                        <div class="flex justify-center w-full border-t border-slate-100 dark:border-slate-700/50 pt-3">
                                            <span class="flex items-center gap-1 text-[10px] md:text-[11px] text-slate-400">
                    ...
                    <template x-if="currentWords().length === 0">
                        <div class="w-full max-w-2xl bg-white dark:bg-slate-800 rounded-[32px] border border-slate-200/50 dark:border-slate-700/60 shadow-lg p-12 text-center flex flex-col items-center justify-center gap-6 relative z-10 my-12">
                            <div class="h-20 w-20 bg-yellow-100 dark:bg-yellow-500/10 rounded-full flex items-center justify-center text-yellow-500 animate-bounce">
                                <span class="material-symbols-outlined text-4xl">workspace_premium</span>
                            </div>
                            <div>
                                <h3 class="text-xl md:text-2xl font-extrabold text-slate-800 dark:text-white" x-text="studyMode === 'level' ? 'Chúc mừng! Bạn đã thuộc hết từ vựng HSK ' + activeLevel : 'Chúc mừng! Bạn đã thuộc hết từ vựng chủ đề ' + activeTopic"></h3>
                                <p class="text-slate-500 dark:text-slate-400 text-xs md:text-sm mt-2">Tuyệt vời! Bạn đã ghi nhớ toàn bộ từ vựng ở phần này.</p>
                            </div>
                        </div>
                    </template>
        ...
        >
            ...
            >
                <!-- Drawer Header -->
                <div class="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-700/80 mb-6">
                ...
                <div class="flex flex-col gap-6 pb-8">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <template x-for="level in levels" :key="level">
                                <button 
                                    @click="changeLevel(level); isFilterDrawerOpen = false"
                    ...
                    <div>
                        ...
                        <div class="flex flex-col gap-2.5">
                            <template x-for="(words, topicName) in topics" :key="topicName">
                                <button 
                                    @click="changeTopic(topicName); isFilterDrawerOpen = false"
                                ...
                                >
                                    <div class="flex items-center gap-3">
                                        <div 
                                            class="flex h-7 w-7 items-center justify-center rounded-full text-xs shadow-sm"
                                            :class="studyMode === 'topic' && activeTopic === topicName 
                                                ? 'bg-white text-primary' 
                                                : 'bg-primary/5 dark:bg-primary/10 text-primary'"
                                        >
                                        ...
                                        <div class="text-left">
                                            <p class="font-bold text-xs" :class="studyMode === 'topic' && activeTopic === topicName ? 'text-white' : 'text-slate-800 dark:text-white'" x-text="topicName"></p>
                                            <p class="text-[9px] font-medium opacity-80" :class="studyMode === 'topic' && activeTopic === topicName ? 'text-white/80' : 'text-slate-400 dark:text-slate-500'" x-text="words.length + ' từ vựng'"></p>
                                        </div>
                                    </div>
                                    <span class="material-symbols-outlined text-[14px]" :class="studyMode === 'topic' && activeTopic === topicName ? 'text-white' : 'text-slate-400 dark:text-slate-500'">chevron_right</span>
                                </button>
                            </template>
                        </div>
                    </div>
