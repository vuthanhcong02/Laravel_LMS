...
<template x-teleport="body">
    <template x-if="isFullscreen">
        <div x-data="pinyinDragScroll"
             @mousedown="initDrag($event)"
             @mouseleave="endDrag($event)"
             @mouseup="endDrag($event)"
             @mousemove="doDrag($event)"
             @click.capture="handleClick($event)"
             class="pinyin-fs-overlay bg-[#f8f6f6] dark:bg-[#0e0c0b] no-scrollbar cursor-grab">
            <div class="sticky top-0 left-0 right-0 z-40 mb-2 flex items-center justify-between px-3 py-2 rounded-xl border shadow-sm bg-white/95 dark:bg-slate-900/95 border-slate-200 dark:border-slate-700">
                <div class="flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full bg-[#e07a5f]"></span>
                    <span class="text-xs font-bold text-slate-800 dark:text-white">{{ __('Bảng Phiên Âm Pinyin — Chế độ toàn màn hình') }}</span>
...
<template x-teleport="body">
    <div x-show="isModalOpen"
        ...
        <div x-show="isModalOpen"
             @click.away="isModalOpen = false; currentPinyin = null; selectedTone = null"
             x-transition:enter="transition ease-out duration-200"
            ...
            <button type="button"
                    @click="isModalOpen = false; currentPinyin = null; selectedTone = null"
            ...
            <div x-show="isLoadingPinyin" class="flex items-center justify-center py-6 gap-3 text-slate-400 text-xs">
                <svg class="animate-spin h-5 w-5 text-[#e07a5f]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                </svg>
                <span>{{ __('Đang tải...') }}</span>
            </div>
            <div class="space-y-2" x-show="!isLoadingPinyin && currentPinyin && currentPinyin.tones && currentPinyin.tones.length > 0">
            ...
            <div x-show="!isLoadingPinyin && currentPinyin && (!currentPinyin.tones || currentPinyin.tones.length === 0)" class="text-center py-6 text-slate-400 text-xs">
