...
@endsection
@section('content')
    <div x-data="pinyinQuizApp({{ $quizTonesJson }})" class="space-y-6 pb-12" @keydown.window="handleGlobalKey($event)">
        {{-- ========================================================================= --}}
        {{-- ========================================================================= --}}
        <div
        ...
        {{-- ========================================================================= --}}
        {{-- ========================================================================= --}}
        <div x-show="screen === 'setup'" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="lms-card p-5 sm:p-6 space-y-4">
                <div class="flex items-center justify-between border-b border-[#e8e2d9] dark:border-[#2d2926] pb-3">
                    ...
                    <span class="text-[11px] font-medium text-slate-400"
                        x-text="`Có ${filteredTones.length} âm sẵn sàng`"></span>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    <button type="button" @click="selectedCategory = 'all'; filterTones()"
                        :class="selectedCategory === 'all' ?
                            'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2c221e] ring-2 ring-[#e07a5f]/30' :
                            'border-[#e8e2d9] dark:border-[#2d2926] bg-[#f8f6f3]/60 dark:bg-[#201d1b]/60 hover:bg-[#fff2ee]/40 dark:hover:bg-[#2c221e]/40'"
                        class="p-3.5 rounded-2xl border text-left transition-all btn-tactile flex items-start gap-3 group cursor-pointer">
                        <div
                            class="w-9 h-9 rounded-xl bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926] text-[#e07a5f] flex items-center justify-center font-bold text-base shrink-0 shadow-xs group-hover:scale-105 transition-transform">
                            🎯
                        </div>
                        <div class="space-y-0.5 min-w-0">
                            <h3
                                class="text-sm font-bold text-slate-800 dark:text-white group-hover:text-[#e07a5f] transition-colors truncate">
                    ...
                    <button type="button" @click="selectedCategory = 'aspirated'; filterTones()"
                        :class="selectedCategory === 'aspirated' ?
                            'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2c221e] ring-2 ring-[#e07a5f]/30' :
                            'border-[#e8e2d9] dark:border-[#2d2926] bg-[#f8f6f3]/60 dark:bg-[#201d1b]/60 hover:bg-[#fff2ee]/40 dark:hover:bg-[#2c221e]/40'"
                        class="p-3.5 rounded-2xl border text-left transition-all btn-tactile flex items-start gap-3 group cursor-pointer">
                        <div
                    ...
                    <button type="button" @click="selectedCategory = 'retroflex'; filterTones()"
                        :class="selectedCategory === 'retroflex' ?
                            'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2c221e] ring-2 ring-[#e07a5f]/30' :
                            'border-[#e8e2d9] dark:border-[#2d2926] bg-[#f8f6f3]/60 dark:bg-[#201d1b]/60 hover:bg-[#fff2ee]/40 dark:hover:bg-[#2c221e]/40'"
                        class="p-3.5 rounded-2xl border text-left transition-all btn-tactile flex items-start gap-3 group cursor-pointer">
                        <div
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="lms-card p-5 space-y-3.5">
                    ...
                    <div class="grid grid-cols-3 gap-2.5">
                        <button type="button" @click="selectQuizLength(10)"
                            :class="(configQuizLength || quizLength) === 10 ?
                                'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2c221e] text-[#e07a5f] font-bold' :
                                'border-[#e8e2d9] dark:border-[#2d2926] bg-[#f8f6f3]/60 dark:bg-[#201d1b]/60 text-slate-700 dark:text-slate-300'"
                        ...
                        <button type="button" @click="selectQuizLength(20)"
                            :class="(configQuizLength || quizLength) === 20 ?
                                'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2c221e] text-[#e07a5f] font-bold' :
                                'border-[#e8e2d9] dark:border-[#2d2926] bg-[#f8f6f3]/60 dark:bg-[#201d1b]/60 text-slate-700 dark:text-slate-300'"
                            class="p-3 rounded-xl border text-center transition-all btn-tactile cursor-pointer">
                        ...
                        <button type="button" @click="selectQuizLength(50)"
                            :class="(configQuizLength || quizLength) === 50 ?
                                'border-[#e07a5f] bg-[#fff2ee] dark:bg-[#2c221e] text-[#e07a5f] font-bold' :
                                'border-[#e8e2d9] dark:border-[#2d2926] bg-[#f8f6f3]/60 dark:bg-[#201d1b]/60 text-slate-700 dark:text-slate-300'"
                            class="p-3 rounded-xl border text-center transition-all btn-tactile cursor-pointer">
                ...
                <div class="lms-card p-5 space-y-3.5">
                    ...
                    <div class="space-y-2.5">
                        <label
                            class="flex items-center justify-between p-2.5 rounded-xl bg-[#f8f6f3] dark:bg-[#201d1b] border border-[#e8e2d9] dark:border-[#2d2926] cursor-pointer">
                            <div class="flex items-center gap-2">
                                <i class="fa-solid fa-bolt text-[#e07a5f] text-xs"></i>
                                <span
                                    class="text-xs font-semibold text-slate-700 dark:text-slate-200">{{ __('Tự động chuyển câu khi trả lời đúng') }}</span>
                            </div>
                            <input type="checkbox" x-model="autoAdvance"
                                class="rounded text-[#e07a5f] focus:ring-[#e07a5f] w-4 h-4 cursor-pointer">
                        </label>
                        <label
                            class="flex items-center justify-between p-2.5 rounded-xl bg-[#f8f6f3] dark:bg-[#201d1b] border border-[#e8e2d9] dark:border-[#2d2926] cursor-pointer">
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    class="text-xs font-semibold text-slate-700 dark:text-slate-200">{{ __('Tự động phát âm thanh khi vào câu mới') }}</span>
                            </div>
                            <input type="checkbox" x-model="autoPlayAudio"
                                class="rounded text-[#e07a5f] focus:ring-[#e07a5f] w-4 h-4 cursor-pointer">
                        </label>
                    </div>
                </div>
        ...
        {{-- ========================================================================= --}}
        {{-- ========================================================================= --}}
        <div x-show="screen === 'quiz'" x-transition:enter="transition ease-out duration-200"
            x-transition:enter-start="opacity-0 scale-98" x-transition:enter-end="opacity-100 scale-100"
            class="space-y-5 max-w-2xl mx-auto" style="display: none;">
            <div
                class="flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-2xl bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926] shadow-xs">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div
                        class="px-3 py-1 rounded-xl bg-[#fff2ee] dark:bg-[#2c221e] border border-[#fcdccf] dark:border-[#4a2e26] text-[#e07a5f] dark:text-[#f4978e] text-xs font-bold flex items-center gap-1.5">
            ...
            <div class="lms-card p-5 sm:p-6 space-y-5 text-center relative overflow-hidden">
                <div x-show="!answered" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full text-xs font-semibold transition-all duration-300"
                        ...
                        <span class="relative flex h-2 w-2">
                            <span x-show="isPlaying"
                                class="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#e07a5f] opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2"
                                :class="isPlaying ? 'bg-[#e07a5f]' : 'bg-slate-400 dark:bg-slate-500'"></span>
                        </span>
                        <span class="tracking-wider uppercase text-[11px] font-bold"
                            x-text="isPlaying ? '{{ __('Đang phát âm thanh...') }}' : '{{ __('Nghe phát âm và chọn đáp án') }}'"></span>
                    </div>
                    <div class="relative flex items-center justify-center my-1">
                        ...
                        <div class="w-22 h-22 sm:w-24 sm:h-24 rounded-full bg-[#f8f6f3] dark:bg-[#201d1b] border border-[#e8e2d9] dark:border-[#2d2926] flex items-center justify-center p-2 transition-all duration-300 shadow-inner"
                            :class="isPlaying ? 'ring-4 ring-[#e07a5f]/20 dark:ring-[#e07a5f]/25 scale-105' :
                                'hover:border-[#e07a5f]/40'">
                            <button type="button" @click="playAudio(1.0)"
                                ...
                                <div x-show="isPlaying" class="flex items-center justify-center gap-1 h-6"
                                    style="display: none;">
                                ...
                                <div x-show="!isPlaying"
                                    class="flex items-center justify-center group-hover:scale-110 transition-transform duration-200">
                                    <svg class="w-6 h-6 sm:w-7 sm:h-7 fill-current drop-shadow-xs" viewBox="0 0 24 24">
                                        <path
                                            d="M14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77zm-2.5-1.73L6.83 6H3c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h3.83l4.67 4.5c.67.65 1.77.18 1.77-.75V2.25c0-.93-1.1-1.4-1.77-.75zM14 8.83v6.34c1.24-.58 2-1.82 2-3.17s-.76-2.59-2-3.17z" />
                                    </svg>
                                </div>
                            </button>
                        </div>
                    ...
                    <div class="flex items-center justify-center pt-0.5">
                        <button type="button" @click="playAudio(1.0)"
                            ...
                            <kbd
                                class="px-1.5 py-0.5 rounded bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926] text-[10px] font-mono text-slate-500 dark:text-slate-400 font-bold">Space</kbd>
                        </button>
                    </div>
                </div>
                <div x-show="answered" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
                    @mouseenter="pauseAdvance()" @mouseleave="resumeAdvance()" class="space-y-3 pt-1 text-left"
                    style="display: none;">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-3 p-3 sm:p-3.5 rounded-2xl border relative overflow-hidden transition-all shadow-xs"
                        ...
                        <div class="flex items-center gap-2.5 w-full sm:w-auto">
                            <div class="shrink-0 w-8 h-8 rounded-xl flex items-center justify-center shadow-2xs"
                                :class="isCorrect ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400' :
                                    'bg-rose-500/15 text-rose-600 dark:text-rose-400'">
                                <template x-if="isCorrect">
                                    <svg class="w-5 h-5 text-emerald-600 dark:text-emerald-400" viewBox="0 0 24 24"
                                        fill="currentColor">
                                        <path fill-rule="evenodd"
                                            d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z"
                                            clip-rule="evenodd" />
                                    </svg>
                                </template>
                                <template x-if="!isCorrect">
                                    <svg class="w-5 h-5 text-rose-600 dark:text-rose-400" viewBox="0 0 24 24"
                                        fill="currentColor">
                                        <path fill-rule="evenodd"
                            ...
                            <div class="min-w-0">
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-xs sm:text-sm block"
                                        x-text="isCorrect ? '{{ __('Chính xác! +10 Điểm 🎉') }}' : '{{ __('Chưa chính xác! ❌') }}'"></span>
                                    <template x-if="isCorrect && isPausedAdvance">
                                        <span
                                            class="text-[10px] px-2 py-0.5 rounded-full bg-emerald-200/70 dark:bg-emerald-800/60 text-emerald-900 dark:text-emerald-200 font-semibold animate-pulse">
                    ...
                    <div
                        class="p-3 rounded-2xl bg-[#f8f6f3] dark:bg-[#201d1b] border border-[#e8e2d9] dark:border-[#2d2926] space-y-2">
                        <div class="flex items-center justify-between">
                            <span
                                class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                                <i class="fa-solid fa-atom text-[#e07a5f]"></i>
                                {{ __('Cấu tạo âm') }}: <span
                                    class="text-slate-900 dark:text-white font-bold zh-text text-sm normal-case"
                                    x-text="targetTone ? formatPinyin(targetTone.display) : ''"></span>
                            </span>
                            <div class="flex items-center gap-2">
                                <span class="text-[11px] font-bold text-[#e07a5f]"
                                    x-text="`Thanh ${targetTone ? targetTone.tone_number : ''}`"></span>
                                <button type="button" @click="playAudio(1.0)"
                        ...
                        <div class="grid grid-cols-3 gap-2 text-center">
                            <div
                                class="p-2 rounded-xl bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926]">
                                <span class="text-[10px] font-medium text-slate-400 block">{{ __('Thanh mẫu') }}</span>
                                <span class="font-bold text-[#e07a5f] text-sm sm:text-base zh-text"
                                    x-text="targetTone && targetTone.initial ? targetTone.initial : '—'"></span>
                            </div>
                            <div
                                class="p-2 rounded-xl bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926]">
                                <span class="text-[10px] font-medium text-slate-400 block">{{ __('Vận mẫu') }}</span>
                                <span class="font-bold text-slate-700 dark:text-slate-200 text-sm sm:text-base zh-text"
                                    x-text="targetTone && targetTone.final ? targetTone.final : '—'"></span>
                            </div>
                            <div
                                class="p-2 rounded-xl bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926]">
                                <span class="text-[10px] font-medium text-slate-400 block">{{ __('Cao độ') }}</span>
                                <span
                                    class="font-bold text-slate-700 dark:text-slate-200 text-xs sm:text-sm truncate block"
                                    x-text="getPitchDescription(targetTone ? targetTone.tone_number : 0)"></span>
                            </div>
                        </div>
                        <template x-if="!isCorrect && selectedOpt">
                            <div
                                class="pt-1.5 border-t border-[#e8e2d9] dark:border-[#2d2926] flex flex-wrap items-center justify-between gap-2">
                                <span
                                    class="text-[11px] font-semibold text-slate-600 dark:text-slate-400">{{ __('So sánh phát âm:') }}</span>
                                <div class="flex items-center gap-1.5">
                                    <button type="button" @click="playSpecificToneAudio(targetTone)"
                                        ...
                                        <i class="fa-solid fa-volume-high text-[10px]"></i>
                                        <span
                                            x-text="`{{ __('Âm đúng:') }} ${targetTone ? formatPinyin(targetTone.display) : ''}`"></span>
                                    </button>
                                    <button type="button" @click="playSpecificToneAudio(selectedOpt)"
                                        ...
                                        <i class="fa-solid fa-volume-high text-[10px]"></i>
                                        <span
                                            x-text="`{{ __('Bạn chọn:') }} ${selectedOpt ? formatPinyin(selectedOpt.display) : ''}`"></span>
                                    </button>
                                </div>
                            </div>
                        </template>
                ...
                <div class="grid grid-cols-2 gap-3 max-w-md mx-auto pt-1">
                    <template x-for="(opt, idx) in currentOptions" :key="opt.id + '_' + idx">
                        <button type="button" @click="selectAnswer(opt)" :disabled="answered"
                            ...
                            <span
                                class="absolute top-2.5 left-2.5 text-[10px] w-5 h-5 rounded-md flex items-center justify-center font-bold"
                                :class="getBadgeClass(opt)" x-text="['A', 'B', 'C', 'D'][idx]"></span>
                            {{-- Pinyin Display --}}
            ...
            <div class="flex items-center justify-center gap-3 text-[11px] text-slate-400">
                <span><kbd
                        class="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-mono text-[10px]">1-4</kbd>
                    / <kbd
                        class="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-mono text-[10px]">A-D</kbd>
                    Chọn đáp án</span>
                <span>•</span>
                <span><kbd
                        class="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-mono text-[10px]">Space</kbd>
                    Nghe lại</span>
                <span>•</span>
                <span><kbd
                        class="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-mono text-[10px]">Enter</kbd>
                    Tiếp tục</span>
            </div>
        </div>
        {{-- ========================================================================= --}}
        {{-- ========================================================================= --}}
        <div x-show="screen === 'summary'" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
            class="space-y-5 max-w-xl mx-auto" style="display: none;">
            <div class="lms-card p-5 sm:p-6 text-center space-y-5">
                {{-- Trophy Icon --}}
                <div
                    class="w-16 h-16 rounded-full bg-[#fff2ee] dark:bg-[#2c221e] text-[#e07a5f] flex items-center justify-center mx-auto border-4 border-white dark:border-[#181615] shadow-lg">
                ...
                <div
                    ...
                    <div class="flex flex-col items-center">
                        <span
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{{ __('Độ chính xác') }}</span>
                        <span class="text-xl font-bold text-[#e07a5f] mt-0.5"
                            x-text="`${Math.round((correctCount / quizLength) * 100)}%`"></span>
                    </div>
                    <div class="flex flex-col items-center">
                        <span
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{{ __('Tổng Điểm') }}</span>
                        <span class="text-xl font-bold text-slate-800 dark:text-white mt-0.5" x-text="score"></span>
                    </div>
                    <div class="flex flex-col items-center">
                        <span
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{{ __('Streak Dài Nhất') }}</span>
                        <span class="text-xl font-bold text-amber-500 mt-0.5" x-text="maxStreak"></span>
                    </div>
                    <div class="flex flex-col items-center">
                        <span
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{{ __('Đúng / Tổng') }}</span>
                        <span class="text-xl font-bold text-emerald-600 mt-0.5"
                            x-text="`${correctCount}/${quizLength}`"></span>
                    </div>
                </div>
                <template x-if="mistakes.length > 0">
                    <div class="space-y-2.5 text-left pt-1">
                        <div
                            class="flex items-center justify-between border-b border-[#e8e2d9] dark:border-[#2d2926] pb-2">
                            <span
                                class="text-xs font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        <div class="space-y-2 max-h-48 overflow-y-auto pr-1 no-scrollbar">
                            <template x-for="(m, idx) in mistakes" :key="idx">
                                <div
                                    class="p-3 rounded-xl bg-[#f8f6f3] dark:bg-[#201d1b] border border-[#e8e2d9] dark:border-[#2d2926] flex items-center justify-between gap-3">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <div>
                                            <div class="flex items-center gap-1.5">
                                                <span class="font-bold text-emerald-600 text-sm zh-text"
                                                    x-text="formatPinyin(m.target.display)"></span>
                                                <span class="text-[11px] text-slate-400 font-medium">(Thanh <span
                                                        x-text="m.target.tone_number"></span>)</span>
                                            </div>
                                            <span class="text-[11px] text-rose-500 font-medium"
                                                x-text="`{{ __('Bạn đã chọn:') }} ${m.chosen ? formatPinyin(m.chosen.display) : '{{ __('Chưa chọn') }}'}`"></span>
                                        </div>
                                    </div>
                                    <span
                                        class="text-[10px] font-bold px-2 py-0.5 rounded bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300"
                                        x-text="`#${idx + 1}`"></span>
                                </div>
                            </template>
                        </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-2">
                    ...
                    <button type="button" @click="startQuiz()" :class="mistakes.length === 0 ? 'sm:col-span-2' : ''"
        ...
        <div x-show="showQuitModal" style="display: none;"
            ...
            <div class="lms-card p-6 bg-white dark:bg-[#181615] w-full max-w-sm relative z-10 shadow-2xl"
                ...
                <div class="flex flex-col items-center text-center space-y-4">
                    <div
                        class="w-14 h-14 rounded-full bg-rose-50 dark:bg-rose-950/40 text-rose-500 flex items-center justify-center border-4 border-white dark:border-[#23201e] shadow-sm">
