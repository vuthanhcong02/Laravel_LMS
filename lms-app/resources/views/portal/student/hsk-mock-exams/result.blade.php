...
@section('title', __('Kết quả thi thử') . ' ' . ($result->mockExam->hskLevel->title ?? 'HSK ' . $level) . ' - ' .
    ($result->mockExam->title ?? '') . ' - XiaoMu LMS')

@section('header-left')
    <x-lms.breadcrumb :links="[
        ['label' => __('Luyện thi HSK'), 'url' => route('student.hsk-mock-exams.index')],
        [
            'label' => $result->mockExam->hskLevel->title ?? 'HSK ' . $level,
            'url' => route('student.hsk-mock-exams.show', ['level' => $level]),
        ],
...
@section('content')
    @php
        ...
        if ($result->completed_at && $result->started_at) {
            $durationSeconds = \Carbon\Carbon::parse($result->started_at)->diffInSeconds(
                \Carbon\Carbon::parse($result->completed_at),
            );
        ...
        $displayOption = function ($opt, $q, $ua = null) {
                        ...
                        if (!empty($parsedEx['options']) && is_array($parsedEx['options'])) {
                            foreach ($parsedEx['options'] as $pOpt) {
                                if (isset($pOpt['letter']) && strtoupper($pOpt['letter']) === strtoupper($textAns)) {
                                    ...
                                    if (!empty($optText)) {
                                        $formatted = function_exists('renderHskRubyText')
                                            ? renderHskRubyText($optText)
                                            : e($optText);
            ...
            $isTrueFalse =
                $q->options->count() == 2 && ($q->options[0]->content === '√' || $q->options[0]->content === '×');
            if ($isTrueFalse) {
            ...
            if ($group && !empty($group->passage_text) && str_starts_with(trim($group->passage_text), '{')) {
                ...
                if (!empty($parsedEx['options']) && is_array($parsedEx['options'])) {
                    $searchLetter =
                        !empty($content) && strlen($content) === 1 && ctype_alpha($content)
                            ? strtoupper($content)
                            : $letter;
                    foreach ($parsedEx['options'] as $pOpt) {
    ...
    <div x-data="hskResultViewer" class="space-y-6 pb-12 relative">
        ...
        <div
            class="lms-card p-6 sm:p-7 relative overflow-hidden bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926]">
            <!-- Subtle background accent aura -->
            <div
                class="absolute -right-10 -bottom-10 w-48 h-48 {{ $isPassed ? 'bg-emerald-500/10' : 'bg-[#e07a5f]/10' }} rounded-full blur-3xl pointer-events-none">
            </div>
            ...
            <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                ...
                <div class="flex items-center gap-5 sm:gap-6 flex-col sm:flex-row text-center sm:text-left">
                    ...
                    <div class="relative w-24 h-24 sm:w-28 sm:h-28 shrink-0 flex items-center justify-center">
                        <svg class="w-full h-full -rotate-90" viewBox="0 0 96 96">
                            ...
                            <circle cx="48" cy="48" r="38" fill="transparent"
                                class="stroke-[#f0eae1] dark:stroke-[#282421]" stroke-width="8" />
                            <!-- Progress Arc -->
                            <circle cx="48" cy="48" r="38" fill="transparent" stroke="{{ $strokeColor }}"
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center text-center">
                            <span
                                class="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white leading-none">
                                {{ $result->total_score }}
                            </span>
                    ...
                    <div class="space-y-1.5">
                        <div
                            class="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-[#fcfaf7] dark:bg-[#23201e] border border-[#e8e2d9] dark:border-[#2d2926] text-[11px] font-semibold text-slate-600 dark:text-slate-300">
                            <span class="font-bold text-[#e07a5f]">{{ strtoupper($levelCode) }}</span>
                            <span>•</span>
                            <span
                                class="truncate max-w-[200px] sm:max-w-xs">{{ $result->mockExam->title ?? __('Đề thi chuẩn') }}</span>
                        </div>

                        <h1
                            class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white flex items-center justify-center sm:justify-start gap-2">
                            @if ($isPassed)
                                <i class="fa-solid fa-circle-check text-emerald-500"></i>
                                <span
                                    class="text-emerald-600 dark:text-emerald-400">{{ __('Chúc mừng! Bạn đã đạt yêu cầu') }}</span>
                            @else
                        ...
                        <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-md">
                            {{ __('Điểm thi đạt') }} <strong
                                class="text-slate-800 dark:text-slate-200 font-bold">{{ $result->total_score }}/{{ $maxScore }}</strong>
                            {{ __('điểm') }}
                            · {{ __('Yêu cầu qua môn') }}: <strong
                                class="text-slate-800 dark:text-slate-200 font-bold">{{ $passScore }}</strong>
                            {{ __('điểm') }}
                        </p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-2.5 sm:gap-3 shrink-0">
                    <!-- Duration Inset -->
                    <div
                        class="p-3.5 sm:p-4 rounded-2xl bg-[#fcfaf7] dark:bg-[#23201e] border border-[#e8e2d9] dark:border-[#2d2926] text-center min-w-[100px] sm:min-w-[115px]">
                        <div
                            class="flex items-center justify-center gap-1.5 text-[11px] font-semibold text-slate-400 dark:text-slate-500">
                    ...
                    <div
                        ...
                        <div class="text-lg sm:text-xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">
                            {{ $correctAnswers }}<span
                                class="text-xs text-slate-400 dark:text-slate-500 font-semibold">/{{ $totalAnswers }}</span>
                        </div>
                        <div class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 font-medium">
                    ...
                    <div
                        ...
                        <div class="w-full bg-slate-200 dark:bg-[#2d2926] h-1.5 rounded-full mt-1.5 overflow-hidden">
                            <div class="bg-[#e07a5f] h-full rounded-full transition-all duration-500"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5">
            ...
            <div class="lms-card p-5 space-y-3.5 bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926]">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-10 h-10 rounded-xl bg-sky-50 dark:bg-sky-950/50 text-[#0284c7] flex items-center justify-center text-base shrink-0">
                            <i class="fa-solid fa-headphones"></i>
                        </div>
                        <div>
                            <h3 class="font-bold text-slate-800 dark:text-white text-sm">{{ __('Phần thi Nghe hiểu') }}
                            </h3>
                            <p class="text-[11px] text-slate-400 dark:text-slate-500 font-medium">
                                {{ __('Listening Section') }}</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <span
                            class="text-xl font-bold text-slate-900 dark:text-white">{{ $result->listening_score }}</span>
                ...
                <div class="space-y-1.5">
                    ...
                    <div
                        class="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                        <span>{{ __('Tỷ lệ đạt được') }}</span>
                        <span
                            class="font-semibold text-slate-700 dark:text-slate-300">{{ $result->listening_score }}%</span>
                    </div>
                </div>
            </div>
            ...
            <div class="lms-card p-5 space-y-3.5 bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926]">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-base shrink-0">
                            <i class="fa-solid fa-book-open-reader"></i>
                        </div>
                        <div>
                            <h3 class="font-bold text-slate-800 dark:text-white text-sm">{{ __('Phần thi Đọc hiểu') }}</h3>
                            <p class="text-[11px] text-slate-400 dark:text-slate-500 font-medium">
                                {{ __('Reading Section') }}</p>
                        </div>
                    </div>
                    <div class="text-right">
            ...
            @if (in_array($levelCode, ['hsk3', 'hsk4', 'hsk5', 'hsk6']))
                <div
                    class="lms-card p-5 space-y-3.5 bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926]">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-xl bg-orange-50 dark:bg-orange-950/50 text-[#e07a5f] flex items-center justify-center text-base shrink-0">
                                <i class="fa-solid fa-pen-nib"></i>
                            </div>
                            <div>
                                <h3 class="font-bold text-slate-800 dark:text-white text-sm">{{ __('Phần thi Viết') }}
                                </h3>
                                <p class="text-[11px] text-slate-400 dark:text-slate-500 font-medium">
                                    {{ __('Writing Section') }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <span
                                class="text-xl font-bold text-slate-900 dark:text-white">{{ $result->writing_score }}</span>
                            <span class="text-xs text-slate-400 dark:text-slate-500 font-semibold">/100</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div
                            ...
                            <span>{{ __('Tỷ lệ đạt được') }}</span>
                            <span
                                class="font-semibold text-slate-700 dark:text-slate-300">{{ $result->writing_score }}%</span>
                        </div>
                    </div>
                </div>
            @else
                <div
                    class="lms-card p-5 flex flex-col items-center justify-center text-center space-y-2 bg-[#fcfaf7] dark:bg-[#23201e] border-dashed border-[#e8e2d9] dark:border-[#2d2926]">
                    <div
                        class="w-8 h-8 rounded-lg bg-slate-100 dark:bg-[#181615] text-slate-400 dark:text-slate-500 flex items-center justify-center text-sm">
                        <i class="fa-solid fa-circle-info"></i>
                    </div>
                    <div>
                        <h3 class="text-xs font-bold text-slate-700 dark:text-slate-300">{{ __('Không có phần Viết') }}
                        </h3>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
            <div class="flex items-center gap-2.5">
                ...
                <span
                    class="px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-[#25211e] border border-[#e8e2d9] dark:border-[#2d2926] text-xs font-bold text-slate-600 dark:text-slate-300">
                    {{ $totalAnswers }} {{ __('câu') }}
                </span>
            </div>
            ...
            <div class="flex items-center gap-2 self-start sm:self-auto flex-wrap">
                ...
                <button @click="mobilePaletteOpen = !mobilePaletteOpen" type="button"
                ...
                <div
                    class="inline-flex p-1 bg-white dark:bg-[#181615] rounded-2xl border border-[#e8e2d9] dark:border-[#2d2926] shadow-2xs">
                    <button @click="filter = 'all'"
                        :class="filter === 'all' ? 'bg-[#e07a5f] text-white shadow-xs font-bold' :
                        ...
                        <span class="px-1.5 py-0.2 rounded-md text-[10px]"
                            :class="filter === 'all' ? 'bg-white/20 text-white' :
                                'bg-slate-100 dark:bg-[#25211e] text-slate-500'">{{ $totalAnswers }}</span>
                    </button>

                    <button @click="filter = 'correct'"
                        :class="filter === 'correct' ? 'bg-emerald-600 text-white shadow-xs font-bold' :
                            'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-semibold'"
                        class="px-3 py-1.5 rounded-xl text-xs transition-all flex items-center gap-1.5">
                        <i class="fa-solid fa-check text-[10px]"></i>
                        <span>{{ __('Đúng') }}</span>
                        <span class="px-1.5 py-0.2 rounded-md text-[10px]"
                            :class="filter === 'correct' ? 'bg-white/20 text-white' :
                                'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400'">{{ $correctAnswers }}</span>
                    </button>

                    <button @click="filter = 'incorrect'"
                        :class="filter === 'incorrect' ? 'bg-rose-600 text-white shadow-xs font-bold' :
                            'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-semibold'"
                        class="px-3 py-1.5 rounded-xl text-xs transition-all flex items-center gap-1.5">
                        <i class="fa-solid fa-xmark text-[10px]"></i>
                        <span>{{ __('Sai / Chưa làm') }}</span>
                        <span class="px-1.5 py-0.2 rounded-md text-[10px]"
                            :class="filter === 'incorrect' ? 'bg-white/20 text-white' :
                                'bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400'">{{ $incorrectAnswers }}</span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div class="lg:col-span-8 space-y-4">
                @foreach ($userAnswersBySection as $sectionName => $userAnswers)
                    ...
                    <div
                        class="lms-card overflow-hidden bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926] shadow-xs">
                        ...
                        <div @click="collapsedSections['{{ $sectionKey }}'] = !collapsedSections['{{ $sectionKey }}']"
                            ...
                            <div class="flex items-center gap-2">
                                <span
                                    class="text-xs font-bold px-2.5 py-0.5 rounded-full {{ $sectionCorrectCount === $sectionTotalCount ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/40' : 'bg-white dark:bg-[#25211e] text-slate-600 dark:text-slate-300 border border-[#e8e2d9] dark:border-[#2d2926]' }}">
                                    {{ __('Đúng') }} {{ $sectionCorrectCount }}/{{ $sectionTotalCount }}
                                    {{ __('câu') }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div x-show="!collapsedSections['{{ $sectionKey }}']"
                            ...
                            @foreach ($userAnswers as $ua)
                                ...
                                <div id="{{ $qAnchorId }}"
                                    ...
                                    :class="highlightedQ === '{{ $qAnchorId }}' ?
                                        'bg-[#fff4ef] dark:bg-[#2c1d18] border-l-4 border-l-[#e07a5f] shadow-inner ring-1 ring-inset ring-[#e07a5f]/40' :
                                    ...
                                    <div class="flex items-start gap-3.5">
                                        <!-- Status Badge Icon -->
                                        <div
                                            class="w-7 h-7 shrink-0 rounded-xl flex items-center justify-center text-white text-xs font-bold shadow-xs mt-0.5 {{ $isCorrect ? 'bg-emerald-500 shadow-emerald-500/20' : 'bg-rose-500 shadow-rose-500/20' }}">
                                            @if ($isCorrect)
                                        ...
                                        <div class="flex-1 min-w-0 space-y-2">
                                            <div class="flex items-center gap-2 flex-wrap">
                                                <span
                                                    class="text-xs font-bold text-slate-900 dark:text-white bg-slate-100 dark:bg-[#25211e] px-2.5 py-0.5 rounded-lg border border-[#e8e2d9] dark:border-[#2d2926]">
                                                    {{ __('Câu') }} {{ $displayQNum }}
                                                </span>
                                                ...
                                                <span
                                                    class="text-[11px] font-bold px-2 py-0.5 rounded-md {{ $isCorrect ? 'text-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 dark:text-emerald-400' : 'text-rose-700 bg-rose-50 dark:bg-rose-950/40 dark:text-rose-400' }}">
                                            ...
                                            @if ($group && !empty($group->passage_audio))
                                                <div class="pt-1 pb-2">
                                                    <div
                                                        class="flex items-center gap-3 p-2.5 rounded-xl bg-slate-50 dark:bg-[#25211e] border border-[#e8e2d9] dark:border-[#2d2926] max-w-md">
                                                        <i
                                                            class="fa-solid fa-volume-high text-indigo-500 text-sm pl-1"></i>
                                            ...
                                            @if ($group && !empty($group->passage_text) && !str_starts_with(trim($group->passage_text), '{'))
                                                <div
                                                    class="p-3.5 rounded-xl bg-[#faf8f5] dark:bg-[#201d1a] border border-[#e8e2d9] dark:border-[#2d2926] text-xs sm:text-sm text-slate-700 dark:text-slate-200 leading-relaxed zh-text font-medium">
                                                    {!! function_exists('renderHskRubyText')
                                                        ? renderHskRubyText($group->passage_text)
                                                        : nl2br(e($group->passage_text)) !!}
                                                </div>
                                            @endif
                                            ...
                                            @if (!empty($question->title))
                                                <div
                                                    class="text-sm sm:text-base font-semibold text-slate-800 dark:text-slate-100 leading-relaxed zh-text pt-0.5">
                                    ...
                                    <div class="sm:pl-10 grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                                        <!-- User Answer Card -->
                                        <div
                                            class="p-3.5 rounded-2xl border {{ $isCorrect ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/50' : 'bg-rose-50/50 dark:bg-rose-950/20 border-rose-200 dark:border-rose-900/50' }}">
                                            <div class="flex items-center justify-between gap-2 mb-1.5">
                                                ...
                                                @else
                                                    <span
                                                        class="inline-flex items-center gap-1 text-[10px] font-bold text-rose-600 dark:text-rose-400">
                                        ...
                                        @if (!$isCorrect && $correctOption)
                                            <div
                                                class="p-3.5 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/50">
                                                <div class="flex items-center justify-between gap-2 mb-1.5">
                                                    <span
                                                ...
                                                <div
                                                    class="text-sm font-bold text-emerald-800 dark:text-emerald-300 zh-text">
                                        ...
                                        @elseif(!$isCorrect && !$correctOption && $question->options->isNotEmpty())
                                            <div
                                                class="p-3.5 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/50">
                                                <span
                                                    class="text-[10px] font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-400 block mb-1.5">
                                                    {{ __('Đáp án chính xác') }}
                                                </span>
                                                <div
                                                    class="text-sm font-bold text-emerald-800 dark:text-emerald-300 zh-text">
            ...
            <aside class="hidden lg:block lg:col-span-4 sticky top-0 self-start">
                <div
                    class="lms-card p-4 sm:p-5 bg-white dark:bg-[#181615] border border-[#e8e2d9] dark:border-[#2d2926] shadow-xs space-y-3.5 max-h-[calc(100vh-2rem)] flex flex-col">

                    <!-- Palette Header -->
                    <div
                        class="flex items-center justify-between pb-3 border-b border-[#e8e2d9] dark:border-[#2d2926] shrink-0">
                        <div class="flex items-center gap-2">
                    ...
                    <!-- Legend Indicator -->
                    <div
                        class="flex items-center justify-between gap-2 p-2 rounded-xl bg-[#faf8f5] dark:bg-[#201d1a] border border-[#e8e2d9] dark:border-[#2d2926] text-[11px] font-semibold shrink-0">
                    ...
                    <div class="space-y-3.5 pt-1 overflow-y-auto pr-1 flex-1 no-scrollbar hover:scrollbar-thin"
                        ...
                        @foreach ($userAnswersBySection as $sectionName => $userAnswers)
                            ...
                            <div class="space-y-1.5">
                                <div
                                    ...
                                    <span
                                        class="uppercase tracking-wider text-[10px] text-slate-400">{{ $sectionName }}</span>
                                    <span
                                        class="text-[10px] px-1.5 py-0.2 rounded-md bg-slate-100 dark:bg-[#25211e] text-slate-500">{{ $userAnswers->count() }}
                                        {{ __('câu') }}</span>
                                </div>
                                ...
                                <div class="grid grid-cols-7 xl:grid-cols-8 gap-1.5 justify-items-center">
                                    @foreach ($userAnswers as $ua)
                                        ...
                                        <button type="button"
                                            ...
                                            :class="highlightedQ === '{{ $qAnchor }}' ?
                                                'ring-2 ring-[#e07a5f] ring-offset-2 ring-offset-white dark:ring-offset-[#181615] scale-110' :
                                                ''"
        ...
        <div x-show="mobilePaletteOpen" x-transition:enter="transition ease-out duration-200"
            ...
            <div @click.away="mobilePaletteOpen = false"
                ...
                <div class="p-4 border-b border-[#e8e2d9] dark:border-[#2d2926] flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-bold text-slate-900 dark:text-white text-sm">{{ __('Bảng câu hỏi bài thi') }}
                        </h3>
                    </div>
                    <button type="button" @click="mobilePaletteOpen = false"
                        class="w-8 h-8 rounded-xl bg-slate-100 dark:bg-[#25211e] text-slate-500 hover:text-slate-800 flex items-center justify-center">
                ...
                <div class="p-4 overflow-y-auto space-y-4">
                    <!-- Legend -->
                    <div
                        class="flex items-center justify-between gap-2 p-2.5 rounded-xl bg-[#faf8f5] dark:bg-[#201d1a] border border-[#e8e2d9] dark:border-[#2d2926] text-xs font-semibold">
                        <span class="text-emerald-600 dark:text-emerald-400">✓ {{ __('Đúng') }}:
                            {{ $correctAnswers }}</span>
                        <span class="text-rose-600 dark:text-rose-400">✕ {{ __('Sai') }}:
                            {{ $incorrectAnswers }}</span>
                    </div>
        ...
        <!-- Bottom Actions Card -->
        <div
            class="lms-card p-6 sm:p-8 bg-gradient-to-r from-[#fff7f4] via-white to-[#fff2ee] dark:from-[#1e1a18] dark:via-[#1c1917] dark:to-[#221c19] border border-[#e8e2d9] dark:border-[#2d2926] text-center space-y-4">
            <div class="space-y-1">
