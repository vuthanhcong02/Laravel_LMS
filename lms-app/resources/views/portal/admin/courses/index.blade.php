            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-sm p-4">
                <form method="GET" action="{{ route('admin.courses.index') }}"
                    ...
                    <div class="flex flex-1 items-center gap-2 px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus-within:ring-2 focus-within:ring-primary/40 transition-all">
                        ...
                        @if (request('search'))
                            <a href="{{ route('admin.courses.index', array_filter(['status' => request('status'), 'category_id' => request('category_id')])) }}"
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase">Teacher</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase text-center">Price</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase text-center">Enrolled</th>
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            @endif
                                            <div>
                                                <p class="font-semibold text-slate-900 dark:text-white text-sm line-clamp-1">
                                                    {{ $course->title }}</p>
                                    ...
                                    <td class="px-6 py-4 text-center text-sm text-slate-700 dark:text-slate-300 font-medium">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if ($course->is_published)
                                            <span class="px-5 py-2 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold">Published</span>
                                        @else
                                            <span class="px-5 py-2 bg-yellow-100 text-yellow-700 rounded-full text-xs font-bold">Draft</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-baseline justify-end gap-3">
                                            ...
                                            <button type="button"
                                                ...
                                                data-message="Are you sure you want to delete &quot;{{ $course->title }}&quot;? This action cannot be undone."
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center">
                                        <span class="material-symbols-outlined text-4xl text-slate-300 block mb-2">school</span>
